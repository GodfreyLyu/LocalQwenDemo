...
All endpoints use JSON. API responses carry `X-Request-ID` and `Cache-Control: no-store`. Source, passwords, cookies, and model responses are never request-log fields. Invalid input errors do not echo Pydantic's input values.
...
```json
{"error":{"code":"queue_full","message":"The review queue is full. Try again later.","request_id":"UUID"}}
```
